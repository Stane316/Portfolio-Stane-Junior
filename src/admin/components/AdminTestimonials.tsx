...
/**
 * AdminTestimonials — CRUD interface for testimonials
 *
    ...
    <div className="space-y-4">
      ...
      {testimonials.length === 0 ? (
        <EmptyState
          ...
          description="Ajoutez des témoignages clients pour renforcer la crédibilité."
          action={{ label: 'Ajouter un témoignage', onClick: () => setShowForm(true) }}
        />
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key={t.id} className="glass-card border border-[rgba(0,191,255,0.15)]">
              <div className="flex items-start gap-3 mb-3">
                ...
                <div className="flex gap-1 flex-shrink-0">
                  {/* Toggle visibility — P-13 FIX: SVG icon replacing 👁/🙈 */}
                  <button onClick={() => toggleVisible(t.id, t.is_visible)} className="p-1.5 text-[#A8B4C8] hover:text-[#00BFFF] transition-colors" aria-label={t.is_visible ? 'Masquer le témoignage' : 'Afficher le témoignage'}>
                    {t.is_visible ? (
                  ...
                  <button onClick={() => handleEdit(t)} className="p-1.5 text-[#A8B4C8] hover:text-[#00BFFF] transition-colors" aria-label="Modifier le témoignage">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                  </button>
                  {/* Delete — P-13 FIX: SVG icon replacing 🗑 */}
                  <button onClick={() => handleDelete(t.id, t.person_name)} className="p-1.5 text-[#A8B4C8] hover:text-red-400 transition-colors" aria-label="Supprimer le témoignage">
