...
const AdminGrowTechMembers: React.FC<AdminGrowTechMembersProps> = ({ members, onChange, onToast }) => {
  ...
  const handleSave = () => {
    ...
    if (editingMember) {
      updated = members.map((m) => m.id === editingMember.id ? newMember : m);
      onToast('success', 'Membre modifié (local)');
    } else {
      updated = [...members, newMember];
      onToast('success', 'Membre ajouté (local)');
    }
  ...
  const handleRemove = (id: string) => {
    ...
    onChange(updated);
    onToast('success', 'Membre supprimé (local)');
  };
  ...
  return (
    <div className="glass-card p-6 space-y-4">
        ...
        {showForm && (
          ...
          >
            ...
            <div className="flex gap-3 mt-3">
              <button 
                onClick={handleSave} 
                className="flex-1 bg-[#00BFFF] text-black font-bold py-2 rounded hover:opacity-90"
              >
                {editingMember ? 'Modifier' : 'Ajouter au brouillon'}
              </button>
              {editingMember && (
      ...
      <div className="space-y-2 mt-4">
        {members.map((member) => (
          <div key={member.id} className="flex items-center justify-between bg-[#141430] p-3 rounded border border-[#1A1A2E]">
            <div className="flex items-center gap-3">
              {member.image_url ? (
                <ImageWithLazyLoad src={member.image_url} alt={member.name} className="w-8 h-8 rounded-full" />
              ) : (
                <div className="w-8 h-8 rounded-full bg-[#0A0A1E] flex items-center justify-center text-xs font-bold text-[#00BFFF]">
                  {member.initial}
                </div>
              )}
              <div>
            ...
            <div className="flex gap-2">
              <button 
                ...
                className="p-1 text-[#A8B4C8] hover:text-[#00BFFF] transition-colors" 
              ...
              <button 
                ...
                className="p-1 text-[#A8B4C8] hover:text-red-400 transition-colors" 
        ...
        {members.length === 0 && <p className="text-[#4A5568] text-center italic py-4">Aucun membre ajouté. Les ajouts restent locaux jusqu'à Enregistrer.</p>}
      </div>
    </div>
  );
};
